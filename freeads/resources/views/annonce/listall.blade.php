@extends('layouts.app')

@section('content')
<section class="search-sec">
    <div class="container">
        <form action="{{url('/annonces/search')}}" method="GET" novalidate="novalidate">
            <div class="row">
                <div class="col-lg-12">
                    <div class="row">
                        <div class="col-lg-2 col-md-2 col-sm-12 p-0">
                            <input name="titre" type="text" class="form-control search-slt" placeholder="Rechercher par Titre">
                        </div>
                        <div class="col-lg-2 col-md-2 col-sm-12 p-0">
                            <input name="type" type="text" class="form-control search-slt" placeholder="Type ex: telephone">
                        </div>
                        <div class="col-lg-2 col-md-2 col-sm-12 p-0">
                            <input name="prix" type="text" class="form-control search-slt" placeholder="Prix maximum">
                        </div>
                        <div class="col-lg-2 col-md-2 col-sm-12 p-0">
                            <input name="ville" type="text" class="form-control search-slt" placeholder="Ville">
                        </div>
                        <div class="col-lg-2 col-md-2 col-sm-12 p-0">
                            <select name="list" class="form-control search-slt" id="exampleFormControlSelect1">
                                <option value="prixc">Prix croissant</option>
                                <option value="prixd">Prix decroissant</option>
                                <option value="date">Date de publication</option>
                            </select>
                        </div>
                        <div class="col-lg-2 col-md-2 col-sm-12 p-0">
                            <button type="submit" class="btn btn-danger wrn-btn">Search</button>
                        </div>
                    </div>
                </div>
            </div>
        </form>
    </div>
</section>
<div class="container-fluid">
@foreach($annonces as $annonce)
    <div class="col-md-2 column productbox">
        @if(isset($annonce->image1))
        <img src="/uploads/annonce/{{$annonce->image1}}" class="img-responsive">
        @else
        <img src="data:image/jpeg;base64," class="img-responsive">
        @endif
        <!-- <img src="http://placehold.it/460x250/e67e22/ffffff&text=HTML5" class="img-responsive"> -->
        <div class="producttitle">{{$annonce->titre}}   <p>{{$annonce->ville}}</p></div>
        <div class="productprice"><div class="pull-right"><a href="{{route('annonce.info',$annonce->id)}}" class="btn btn-danger btn-sm" role="button">En savoir plus</a></div><div class="pricetext">{{$annonce->prix}} €</div></div>
    </div>
@endforeach
</div>
@endsection
